...
cut -d\, -f4,5,17  $sStatePopulationDataPath | tail -n+2 | sed "1 s|^|COUNTY_FIPS,POPULATION_2017\n|g" | sed "2,$ s|\,||1" > temp.5.csv
joiner 'COUNTY_FIPS' temp.4.csv temp.5.csv > temp.6.csv
rm -f temp.7.db
sqlite3 temp.7.db ".import $sIODir/temp.6.csv tbl1"
sqlite3 temp.7.db "select *, cast(IMMIGRATION_RAW as real)/cast(POPULATION_2017 as real) as IMMIGRATION_RATE from tbl1;" > temp.8.csv
...
#finding averages for each observation
...
for i in {1..539}
do
	sqlite3 temp.19.db "select '$i' as IMMIGRATION_RANK, * from tbl1 where cast(RANK_START as real)<=$i and $i<=cast(RANK_END as real);" | tail -n+2 >> temp.20.csv
done
rm -f temp.21.db
sqlite3 temp.21.db ".import $sIODir/temp.20.csv tbl1"
sqlite3 temp.21.db "select IMMIGRATION_RANK, THRESHOLD, avg(cast(OBSERVED as real)) as OBSERVED_MEAN, avg(cast(SES as real)) as SES_MEAN from tbl1 group by IMMIGRATION_RANK, THRESHOLD;" > temp.22.csv 
...
sqlite3 temp.27.db "select SAMPLE_ID, COUNTY_FIPS, LATITUDE, LONGITUDE, IMMIGRATION_RATE, OBSERVED_MEAN, THRESHOLD from tbl1;" > temp.27.csv

java -cp $sJavaDir/Utilities.jar edu.ucsf.FlatFileToPivotTable.FlatFileToPivotTableLauncher \
	--sDataPath=$sIODir/temp.27.csv \
	--sValueHeader=OBSERVED_MEAN \
